...
```
[Node-RED] → 2초마다 센서 데이터 생성
     ↓
[MySQL] ← 센서 수치 저장 (온도, 습도, 토양수분, CO2, 조도)
     ↓
[Django REST API] ← MySQL에서 데이터 읽어 API 제공
     ↓
[Ollama AI] ← 이상값 감지 및 분석
     ↓
[MongoDB] ← 로그 및 AI 분석 결과 저장
     ↓
[n8n] ← 이상값 감지 시 자동 알림 워크플로우
     ↓
[PHP 대시보드 / 브라우저] ← 실시간 모니터링
```
...
예: 온도 45.8°C 감지 → `alert_count` 증가 → n8n 자동 알림 발송
